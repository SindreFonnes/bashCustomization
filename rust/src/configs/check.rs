...
use crate::configs::state::{SelfManagedEntry, detect_state, load_self_managed, prune_stale_self_managed, remove_self_managed};
...
mod tests {
    ...
    fn check_silent_when_all_linked() {
        ...
        assert!(output.is_empty(), "output should be empty when all linked, got: {output:?}");
    ...
    fn check_silent_when_self_managed() {
        ...
        assert!(output.is_empty(), "output should be empty for self-managed, got: {output:?}");
    ...
    fn check_auto_links_multiple_entries_and_dedups_names_in_summary() {
        ...
        write_check(&mut buf, &entries, &unfiltered, &[], dir.path())
            .expect("write_check failed");
        ...
        assert!(claude_pos < zellij_pos, "claude should appear before zellij (alphabetical)");

        // Should not have "claude" twice
        let claude_count = output.matches("claude").count();
        assert_eq!(claude_count, 1, "claude should appear only once (deduped), got: {output:?}");
    }
    ...
    fn check_warns_on_wrong_symlink_without_modifying() {
        ...
        assert!(output.contains("wrong symlink"), "output should mention 'wrong symlink', got: {output:?}");
    }
    ...
    fn check_prunes_marker_when_entry_no_longer_in_manifest() {
        ...
        let mut buf: Vec<u8> = Vec::new();
        write_check(&mut buf, &filtered, &unfiltered, &sm, dir.path())
            .expect("write_check failed");
        let output = String::from_utf8(buf).unwrap();
    ...
    fn check_preserves_marker_for_other_platform_entry() {
        ...
        let remaining = load_self_managed(dir.path()).unwrap();
        assert_eq!(remaining.len(), 1, "cross-platform marker should be preserved");
    ...
    fn check_returns_ok_even_with_unresolved_drift() {
        ...
        assert!(result.is_ok(), "write_check should return Ok even with unresolved drift");
    }
}
